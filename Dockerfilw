...
RUN wget https://artifactory.dev.maximus.axisb.com/artifactory/general/certs/dev-wildcard-2019/root.cer --no-check-certificate -P /etc/pki/ca-trust/source/anchors/ && update-ca-trust extract


Here are the observations so far:

1. jenkins to bitbucket via blueocean - DONE
2. git clone inside master jenkins - NOT WORKING