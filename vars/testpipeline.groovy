def deleteExistingHelmChart() {
    container('jenkins-agent') {
        withFolderProperties {
            sh '''
            helm repo add stable https://artifactory.test.cicd.com/artifactory/forex-helm-virtual
        ...
        container('jenkins-agent') {
            script {
                if("${ENVIRONMENT_NAME}" == "sit") {
                    def app = sh(returnStdout: true, script: "curl -k 'https://artifactory.test.cicd.com/artifactory/forex-generic/services-build-versions/seed/version.txt'")
                    env.SEED_VERSION = app.split('=')[1]
                }
                else {
                    def upstream_build_number = UPSTREAM_BUILD.split('#')[1]
                    def app = sh(returnStdout: true, script: "curl -k https://artifactory.test.cicd.com/artifactory/forex-generic/${upstreamEnv}-verified/seed/version-${upstream_build_number}.txt")
                    env.SEED_VERSION = app.split('=')[1]
                }
            }
        }
...
def publishVersion() {
    withFolderProperties {
        container('jenkins-agent') {
            ...
            curl -k -u ${ARTIFACTORY_CREDENTIALS_USR}:${ARTIFACTORY_CREDENTIALS_PSW} -T version-latest.txt https://artifactory.test.cicd.com/artifactory/forex-generic/${ENVIRONMENT_NAME}-verified/seed/version-latest.txt
            curl -k -u ${ARTIFACTORY_CREDENTIALS_USR}:${ARTIFACTORY_CREDENTIALS_PSW} -T version-${BUILD_NUMBER}.txt https://artifactory.test.cicd.com/artifactory/forex-generic/${ENVIRONMENT_NAME}-verified/seed/version-${BUILD_NUMBER}.txt
            '''
        }
    }
}
