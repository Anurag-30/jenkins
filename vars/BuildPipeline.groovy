def call(Map pipelineParams) {
    ...
    pipeline {
        ...
        environment {
            ARTIFACTORY_CREDENTIALS = credentials('jfrog-user') // Credentials ID in Jenkins.  
            ...
            stage('build app') {
                steps {
                    container('jenkins-agent') {
                        ...
                        script {
                            env.Version = sh(returnStdout: true, script: 'sh git_version.sh -v gradle').trim()  // Assigning environment variable
                        }
                    }
                }
            }
            ...
            stage('Helm package and push') {
                steps {
                    container('jenkins-agent') {
                        ...
                        sh 'curl -k -u ${ARTIFACTORY_CREDENTIALS_USR}:${ARTIFACTORY_CREDENTIALS_PSW} -T ${SERVICE}-${Version}.tgz https://artifactory.test.cicd.com/artifactory/anurag-helm/Application/${SERVICE}/${SERVICE}-${Version}.tgz '
                    }
                }
            }
        ...
        post {
            always {
                // publish test reports which are in HTML format only. If you want to post all the artifacts irrespeective of type see commonstages.groovy for PublishArtifacts function
                publishHTML target:[
                    allowMissing: false,
